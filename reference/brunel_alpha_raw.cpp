 ...
 *
 *  This file is deliberately unhelped. Nothing here is wrapped, shortened or
 *  made safer. It is the control against which the drafted C++ frontend in
 *  ../draft is compared, and it is the evidence that the kernel API can in
 *  fact be driven from an ordinary C++ program.
 *
 *  It is a translation of pynest/examples/brunel_alpha_nest.py from
 *  NEST 3.10.0 (commit acca9704d). Every parameter is taken from that file.
 *
 ...
 *
 * The Python example calls scipy.special.lambertw with k = -1. NEST's own
 * build does not require GSL, so rather than add a dependency for one scalar
 * we solve w * exp(w) = x by Halley iteration. The branch is selected by the
 * starting point, which lies below -1.
 */
double
lambert_wm1( const double x )
{
  ...
  double w = l1 - l2; // asymptotic start, already below -1 for our argument
 ...
 * used to normalise the synaptic weights. Same expression as ComputePSPnorm
...
{
  // ---------------------------------------------------------------------
  // The kernel must be brought up by hand. There is no RAII handle for it,
  // and shutdown_nest() must be reached on every path or MPI is left dangling.
  // ---------------------------------------------------------------------
  ...
  // --- kernel status -----------------------------------------------------
  // Values enter through a std::variant. Its converting constructor does the
  // sensible thing for the two cases that matter here: a string literal is
  // stored as std::string and an int is stored as long (both checked against
  // the variant index). What it cannot do is tell you that a key is misspelled
  // or that the kernel wanted a double where you gave it an integer.
  Dictionary kernel_status;
  kernel_status[ "resolution" ] = dt;
  kernel_status[ "print_time" ] = true;
  kernel_status[ "overwrite_files" ] = true;
  nest::set_kernel_status( kernel_status );

  // --- neuron parameters -------------------------------------------------
  Dictionary neuron_params;
  neuron_params[ "C_m" ] = c_mem;
  neuron_params[ "tau_m" ] = tau_mem;
  neuron_params[ "tau_syn_ex" ] = tau_syn;
  neuron_params[ "tau_syn_in" ] = tau_syn;
  neuron_params[ "t_ref" ] = 2.0;
  neuron_params[ "E_L" ] = 0.0;
  neuron_params[ "V_reset" ] = 0.0;
  neuron_params[ "V_m" ] = 0.0;
  neuron_params[ "V_th" ] = theta;

  std::printf( "Building network\n" );

  // --- nodes -------------------------------------------------------------
  // nest::create() takes no parameter dictionary. Unlike nest.Create(...,
  // params=...) in Python, the parameters must be applied in a second step,
  // and set_nc_status() takes a *vector* of dictionaries by non-const
  // reference, so the vector cannot be a temporary.
  ...
  Dictionary rec_ex;
  rec_ex[ "label" ] = std::string( "brunel-cpp-ex" );
  rec_ex[ "record_to" ] = std::string( "ascii" );
  std::vector< Dictionary > rec_ex_vec { rec_ex };
  nest::set_nc_status( espikes, rec_ex_vec );

  Dictionary rec_in;
  rec_in[ "label" ] = std::string( "brunel-cpp-in" );
  rec_in[ "record_to" ] = std::string( "ascii" );
  ...
  syn_ex_defaults[ "weight" ] = J_ex;
  syn_ex_defaults[ "delay" ] = delay;
  nest::copy_model( "static_synapse", "excitatory", syn_ex_defaults );

  Dictionary syn_in_defaults;
  syn_in_defaults[ "weight" ] = J_in;
  syn_in_defaults[ "delay" ] = delay;
  nest::copy_model( "static_synapse", "inhibitory", syn_in_defaults );

  // In Python a synapse specification may be the bare string "excitatory".
  // At this level it must always be a dictionary, and the connectivity rule
  // must always be named explicitly: there is no all_to_all default here.
  Dictionary all_to_all;
  all_to_all[ "rule" ] = std::string( "all_to_all" );

  Dictionary syn_ex;
  syn_ex[ "synapse_model" ] = std::string( "excitatory" );
  const std::vector< Dictionary > syn_ex_vec { syn_ex };

  Dictionary syn_in;
  syn_in[ "synapse_model" ] = std::string( "inhibitory" );
  ...
  // nodes_ex[:N_rec]. Note the indexing convention, which nest.h does not
  // state: `start` is 1-based, `stop` is an inclusive 1-based bound, and the
  // step is mandatory. Passing the natural 0-based (0, N_rec) compiles and
  // then throws nest::BadParameter("start < stop required.") at run time.
  ...
  // nest.h, so this one line is the reason the example needs a second header.
  nest::NodeCollectionPTR all_nodes = nodes_ex + nodes_in;

  Dictionary conn_ex;
  conn_ex[ "rule" ] = std::string( "fixed_indegree" );
  conn_ex[ "indegree" ] = CE;
  nest::connect( nodes_ex, all_nodes, conn_ex, syn_ex_vec );

  Dictionary conn_in;
  conn_in[ "rule" ] = std::string( "fixed_indegree" );
  conn_in[ "indegree" ] = CI;
  ...
  // per node, holding std::monostate where the node is not local. So reading
  // a single integer off a single-node collection costs a get<AnyVector>(),
  // an .at(), and a std::get<long>() on the variant. Asking for the obvious
  ...
  const long events_ex = std::get< long >( ex_status.get< AnyVector >( "n_events" ).at( 0 ) );
  const long events_in = std::get< long >( in_status.get< AnyVector >( "n_events" ).at( 0 ) );
  ...
  const long num_synapses = def_ex.get< long >( "num_connections" ) + def_in.get< long >( "num_connections" );
