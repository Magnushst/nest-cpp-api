...
main( int argc, char* argv[] )
{
  ...
  try
  {
    ...
    close_to(
      api::math::conditional( api::math::compare( two, three, api::math::Comparator::less ), two, three ).value(),
      2.0,
      "conditional" );
    ...
    close_to(
      api::get_connections( { { names::source, sources.handle() } } ).get_all< double >( names::weight ).front(),
      4.5,
    ...
    api::cleanup();
    close_to(
      api::get< double >( kernel.status(), names::biological_time ), 40.0, "run continues where simulate left off" );
